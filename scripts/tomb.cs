...
public partial class tomb : CharacterBody2D
{
	...
	private List<DialogueEntry> dialogues = new List<DialogueEntry>
	{
		new DialogueEntry { 
			Name = "It Reads", 
			Text = "Here lies Jim Bob, a fallen explorer", 
			Delay = 2f, 
			Animation = "dirty", 
			Choices = new List<(string, int, string, int)>
			
			{
				("Pay Respects and clean grave", +2, "Happy", 1),
				("Ignore and move on", -2, "Angry", 2),
			}
		},
		...
		new DialogueEntry { Name = "Grave", Text = "You should have paid your respects...", Delay = 2f, Animation = "dirty", Choices = new List<(string, int, string, int)>() }
	};
